...
class BasicNavigator(Node):
    ...
    def goToPose(self, pose, behavior_tree=''):
        ...
        self.result_future = self.goal_handle.get_result_async()
        self.info("result_future from gotoPose,"+str(self.result_future.result()))
        return True
    ...
    def cancelNavigation(self):
        ...
        self.info('Canceling current Navigation.')
        if self.result_future:
            future = self.goal_handle.cancel_goal_async()
            rclpy.spin_until_future_complete(self, future)
        return

    def isNavigationComplete(self):
        """Check if the Navigation request of any type is complete yet."""
        if not self.result_future:
            # Navigation was cancelled or completed
            return True
        rclpy.spin_until_future_complete(self, self.result_future, timeout_sec=3.00)
        if self.result_future.result():
            self.status = self.result_future.result().status
            if self.status != GoalStatus.STATUS_SUCCEEDED:
                self.debug(f'Navigation with failed with status code: {self.status}')
                return True
        else:
            # Timed out, still processing, not complete yet
            return False

        self.debug('Navigation succeeded!')
        return True
    ...
    def _getPathImpl(self, start, goal, planner_id='', use_start=False):
        ...
        while not self.compute_path_to_pose_client.wait_for_server(timeout_sec=10.0):
            self.info("'ComputePathToPose' action server not available, waiting...")

        goal_msg = ComputePathToPose.Goal()
        goal_msg.start = copy.deepcopy(start)
        self.info("frame id of start"+str(start.header.frame_id))
        goal_msg.goal = goal
