...
class FrontierExplorer(Node):
    ...
    def __init__(self):
        ...
        self.camera_orient_goal=PoseStamped()
        self.drl = DrlAgent()
        #self.drl = None
        ...
        #self.camera_depth_subs= self.create_subscription(Image, '/camera/depth/image_rect_raw', self.cb_depth_image_subs, qos_profile= 10, callback_group= ReentrantCallbackGroup())
        ...
        self.frontier_pose_publisher = self.create_publisher(PoseStamped, '/valid_goal_pose', 5, callback_group= ReentrantCallbackGroup())
    ...
    def cb_depth_image_subs(self,msg):
        camera_frame=msg.header.frame_id  
        depth_data = msg.data
        self.camera_depth_image = np.array(depth_data)     
        ...
        while not reachable:
                ...
                for goal in goals:
                    ...
                    if self.navigator.getPath(self.initial_pose, self.trial_goal) is not None:
                        ...
                        self.frontier_pose_publisher.publish(self.trial_goal)
                        best_goal = goal
                        break
                ...
                rank += 1
        return status
    def check_obs_camera(self, goals):
        out_goals = []
        for goal in goals:
            rl_goal_pose= [float(goal[0]), float(goal[1]), self.initial_pose.pose.position.z]
            x, y, z, w = self.helper.rot2quatarnion(current_pose=self.current_pose, next_pose=rl_goal_pose)
            #check the image here 
            self.camera_orient_goal.pose.position.x = self.initial_pose.pose.position.x
            self.camera_orient_goal.pose.position.x = self.initial_pose.pose.position.y
            self.camera_orient_goal.pose.position.x = self.initial_pose.pose.position.z
            self.camera_orient_goal.pose.orientation.x =  x
            self.camera_orient_goal.pose.orientation.y =  y
            self.camera_orient_goal.pose.orientation.z =  z
            self.camera_orient_goal.pose.orientation.w =  w
            self.rotator_success=False
            
            self.call_rotator(input_pose=self.camera_orient_goal)
            if (self.rotator_success==True):
                # modify or reject frontiers
                dist = (self.current_pose[0]-goal[0]) * (self.current_pose[0]-goal[0])+ (self.current_pose[1]-goal[1]) * (self.current_pose[1]-goal[1])
                dist = np.sqrt(dist)
                min_dist = np.min(self.camera_depth_image)
                if min_dist <= dist:
    ...
    def get_reachable_goal_drl(self):
        ...
        while not reachable:
                ...
                #goals = self.check_obs_camera(goals)
                for goal in goals:
                    self.trial_goal.header.frame_id = 'map'
                    self.trial_goal.header.stamp = self.get_clock().now().to_msg()
